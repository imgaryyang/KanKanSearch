...
import com.kankanews.search.db.model.AppSearchResult;
import com.kankanews.search.utils.JacksonUtil;

public class AppQueryService {
	Logger logger = Logger.getLogger(AppQueryService.class);
	...
	public Map<String, Object> search(String queryStr, int page, int rows,
		...
		List<AppSearchResult> queryResult = new ArrayList<AppSearchResult>();
		...
		for (SolrDocument solrDocument : rsp.getResults()) {
			AppSearchResult app = new AppSearchResult(solrDocument);
			if (isHighLight) {
				...
				if (highFields != null && highFields.get("title") != null
					...
					app.setTitle(map.get(solrDocument.get("docId"))
	...
	public Map<String, Object> searchGroup(String queryStr, int page, int rows,
		...
		try {
			response = solrClientApp.query(query);
		} catch (Exception e) {
			logger.error(e.getLocalizedMessage());
			return result;
		}
		...
		if (groupResponse != null) {
			for (GroupCommand groupCommand : groupResponse.getValues()) {
				...
				for (Group group : groupCommand.getValues()) {
					...
					AppSearchResult app = new AppSearchResult(list.get(0));
					if (isHighLight) {
